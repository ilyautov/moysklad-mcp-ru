...
- **`ms_write_method`** / **`ms_delete_method`** — записать и удалить, с подтверждением.
- **`ms_get_raw`** / **`ms_write_raw`** / **`ms_delete_raw`** — любой путь API, даже вне каталога.
...
- Это **alpha**: курированное ядро выверено боем, импортированные из спеков методы
  подтверждай по докам или зови через raw-инструменты.

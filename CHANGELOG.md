...
### Fixed
- **Установленная команда молчала на любой аргумент.** Точкой входа пакета был
  `server:main`, который сразу уходит в stdio-транспорт и argv не разбирает:
  `moysklad-mcp-ru --help` завершался с кодом 0 и пустым выводом. Для человека,
  который только что поставил пакет, это неотличимо от поломки. Точка входа
  теперь `server:cli`.

### Added
- **`moysklad-mcp-ru doctor`** — проверка установки без сети и без токена:
  версия пакета, версия Python, число инструментов, размер каталога, путь к
  стору кабинетов, состояние гейта записи. Раньше селфчек существовал только в
  `serve.py`, а он пакетом не ставится, то есть у поставивших через `uvx`
  (путь, который рекламируют реестр и каталоги) способа проверить установку не
  было вовсе.
- `--version` и `--help` у той же команды.
- Иконка `assets/logo.png` 400x400 для каталогов, показывающих сервер плиткой.

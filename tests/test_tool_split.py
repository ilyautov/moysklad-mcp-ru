...
SPECS = [
    EndpointSpec(operation_id="r", method="GET", host="api.moysklad.ru",
                 path="/v1/read", safety="read"),
    EndpointSpec(operation_id="w", method="POST", host="api.moysklad.ru",
                 path="/v1/write", safety="write"),
    EndpointSpec(operation_id="d", method="DELETE", host="api.moysklad.ru",
    ...
    EndpointSpec(operation_id="lying_delete", method="DELETE",
                 host="api.moysklad.ru", path="/v1/oops", safety="read"),
]


def _server():
    cfg = ServiceConfig(
        name="ms", scheme="https", fields=["token"],
        env_map={"token": "MOYSKLAD_TOKEN"},
        build_headers=lambda c: {}, allowed_host_suffixes=[".moysklad.ru"])
    mcp = FastMCP("test")
    register_generic_tools(mcp, svc="ms", client=MarketplaceClient(cfg),
                           catalog=Catalog(SPECS, default_host="api.moysklad.ru"))
    return mcp
...
def test_six_executors_registered():
    ...
    assert {"ms_call_method", "ms_write_method", "ms_delete_method",
            "ms_get_raw", "ms_write_raw", "ms_delete_raw"} <= names
    # Прежний catch-all не должен вернуться: каталог коннекторов его отклоняет.
    assert "ms_call_raw" not in names


def test_read_tools_are_annotated_read_only():
    t = _tools(_server())
    for name in ("ms_call_method", "ms_get_raw", "ms_fetch_all"):
...
def test_destructive_tools_are_annotated_destructive():
    t = _tools(_server())
    for name in ("ms_delete_method", "ms_delete_raw"):
...
def test_read_tool_refuses_a_delete_mislabelled_as_read():
    ...
    out = _call(_server(), "ms_call_method", {"operation_id": "lying_delete"})
    assert out["error_type"] == "safety_gate"
    assert out["details"]["use_tool"] == "ms_write_method"
    assert out["details"]["http_call_skipped"] is True
...
def test_write_tool_refuses_a_destructive_operation():
    out = _call(_server(), "ms_write_method",
                {"operation_id": "d", "confirm_write": True})
    assert out["details"]["use_tool"] == "ms_delete_method"


def test_delete_tool_refuses_a_plain_write():
    out = _call(_server(), "ms_delete_method",
...
def test_write_tool_still_needs_confirmation():
    out = _call(_server(), "ms_write_method", {"operation_id": "w"})
    assert out["error_type"] == "safety_gate"
    assert "confirm_write=true" in out["details"]["required"]


def test_delete_tool_needs_both_confirmations():
    out = _call(_server(), "ms_delete_method", {"operation_id": "d"})
...
def test_raw_read_tool_refuses_an_unsafe_verb():
    out = _call(_server(), "ms_get_raw", {"path": "/v1/x", "method": "DELETE"})
    assert out["details"]["use_tool"] == "ms_delete_raw"


def test_raw_write_tool_refuses_delete():
    out = _call(_server(), "ms_write_raw",
...
def test_raw_write_tool_refuses_get():
    ...
    assert out["details"]["use_tool"] == "ms_get_raw"
